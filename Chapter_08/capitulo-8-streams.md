...
En este capítulo aprenderemos a comunicarnos por red con asyncio usando **streams**: la API de alto nivel que abstrae todo lo complejo de trabajar con sockets. El recorrido es de lo más bajo a lo más alto:

1. **Transportes y Protocolos** — capa base, basada en callbacks, que envuelve los sockets.
2. **StreamReader / StreamWriter** — capa de alto nivel para leer y escribir datos de forma asíncrona.
3. **Stdin como stream** — cómo leer del teclado de forma no bloqueante.
4. **Creación de servidores** — `asyncio.start_server` para gestionar múltiples clientes concurrentes.
5. **Aplicación real** — un servidor y cliente de chat completo en un solo hilo.
...
Una diferencia importante respecto a lo que hemos visto antes es que estas APIs funcionan con un **diseño basado en callbacks**: en lugar de esperar activamente los datos, asyncio llama a un método de nuestra clase cuando los datos ya están disponibles. Nosotros simplemente definimos qué debe ocurrir cuando eso suceda.
...
Un **transport** (transporte) es una abstracción para comunicarse con un flujo de datos arbitrario. Cuando trabajamos con un socket o con la entrada estándar, necesitamos hacer siempre las mismas operaciones básicas: leer, escribir y cerrar. El transport define exactamente estas operaciones.
...
Un **protocol** (protocolo) aquí no se refiere a HTTP o FTP, sino a una **clase Python** que define qué hacer cuando ocurren ciertos eventos en la conexión:
...
Cuando ejecutamos `await start_server(...)`, obtenemos un objeto `AbstractServer`. Podemos usarlo como **context manager asíncrono** (`async with server:`) para que se cierre correctamente al salir. El método `serve_forever()` mantiene el servidor activo hasta que lo terminamos.

> **Diferencia con un servidor síncrono:** un servidor síncrono tradicional aceptaría una conexión, la procesaría completamente y solo entonces aceptaría la siguiente. Con asyncio y `start_server`, cada nueva conexión genera una nueva coroutine que se ejecuta concurrentemente con todas las demás. El servidor puede atender miles de clientes simultáneos con un solo hilo.
